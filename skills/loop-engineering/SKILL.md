...
どの層でも検証されないまま消える([`reference/lessons.md`](reference/lessons.md) の抽象化の死角)。
...
**その証明対象の関数が実際の呼び出し経路から呼ばれているか**は証明系の関心事の外([`reference/lessons.md`](reference/lessons.md) の
...
**実行ゲートで追加確認する2点**(いずれも [`reference/lessons.md`](reference/lessons.md) の実地教訓):
