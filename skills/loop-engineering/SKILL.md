...
- `tlc` / `sany`：TLA+ のモデル検査器とパーサ。
- `apalache-mc`：型チェック + 記号モデル検査(無限状態向け)。
...
漏れは各層の内側ではなく、この入口に落ちる(実例は [`reference/lessons.md`](reference/lessons.md))。
...
> survivor の大半は **equivalent-mutant**(kill 不能)。見分け方・判定手順・打ち切り基準は [`reference/mutation-oracle.md`](reference/mutation-oracle.md)。「真の安全性 survivor が 0」で打ち切ってよい。
...
> 生成 `.feature` を実行可能テストにする配線(言語別 OSS ランナー、C の例外、緑/赤での生死実証)は [`reference/gherkin-runners.md`](reference/gherkin-runners.md)。
...
このスキルでの作業を完了扱いにする前、または成果を「設計を検証した／網羅した」と報告する前に、必ず `hymme:skill-compliance-reviewer` サブエージェントへ渡して外側から検査させる。
渡すもの: 対象スキル名(loop-engineering)、0段の抽出台帳とトレーサビリティ表のパス、本体ソースの場所。
...
- **真の survivor**(到達状態を変えるのに緑=設計の穴)を残したまま「設計検証済み」と言わない。一方 equivalent-mutant を 0 にしようと無限に粘らない([`reference/mutation-oracle.md`](reference/mutation-oracle.md))。
