---
name: whitebox-dataflow-loop
description: >
  ホワイトボックス構造網羅のうち、ループの反復回数とデータフローを網羅対象にする技法を扱う。
  ループテスト(0回/1回/最大/境界回)、データフローテスト(def-use 連鎖、変数の定義から
  使用への到達経路)を扱う。test-catalog の手法カタログの一部。オフバイワン・空入力・
  上限超過といったループ固有の欠陥を狙いたい、未初期化使用や再代入で上書きされる前提の
  ロジックなどデータの流れに起因する欠陥を狙いたいときに使う。通常は test-catalog スキルの
...
その溝を埋める手法は別ファイル（[mutation-testing.md](../mutation-testing/SKILL.md)）で扱う。

命令・分岐・条件・経路といった制御フロー系の網羅（C0/C1、条件網羅、判定/条件網羅、MC/DC、多重条件網羅、パスカバレッジ、基底パステスト）は [whitebox-controlflow-basic.md](../whitebox-controlflow-basic/SKILL.md) と [whitebox-controlflow-path.md](../whitebox-controlflow-path/SKILL.md) にまとめた。
...
命令・分岐・条件・経路を網羅対象にする手法（C0/C1、条件網羅、判定/条件網羅、MC/DC、多重条件網羅、パスカバレッジ、基底パステスト）は、[whitebox-controlflow-basic.md](../whitebox-controlflow-basic/SKILL.md) と [whitebox-controlflow-path.md](../whitebox-controlflow-path/SKILL.md) にまとめた。
...
テスト自身の欠陥検出力を測る手法は、[mutation-testing.md](../mutation-testing/SKILL.md) で扱う。
