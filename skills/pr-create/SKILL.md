...
1. 生成した本文を一時ファイルに書き出す（セッションの scratchpad 等、リポジトリ外の一時領域）。
2. テンプレ本体と照合する:
...
2. §1 の PUSH STATUS が WARNING（未 push／未 push コミットあり）なら、**AskUserQuestion で push 可否の承認**を取る（要点と選択肢は本文にも記載する）。
