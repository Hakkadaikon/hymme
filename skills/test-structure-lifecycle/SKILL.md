---
name: test-structure-lifecycle
description: >
  リソースのライフサイクル管理とテストデータ供給の型。Four-Phase Test(setup/exercise/
  verify/teardown)、Test Data Builder/Object Mother を扱う。test-catalog の手法カタログの
  一部。DB 接続や一時ファイルなど確保したリソースの後始末をどう書くか、複雑なフィクスチャの
  組み立てを差分指定で簡潔にしたいときに使う。通常は test-catalog スキルの索引経由で
...
テスト本体の内部構成(Arrange/Act/Assert などの段の切り方)は [`test-structure-anatomy.md`](../test-structure-anatomy/SKILL.md) を参照。
...
AAA([`test-structure-anatomy.md`](../test-structure-anatomy/SKILL.md))に後始末の段を明示的に足した型である。
...
### 落とし穴
- teardown を書き忘れると、確保したリソースが次のテストへ漏れ、テスト間で状態が汚染される。実行順に依存して落ちる flaky の温床になる(テスト間順序・状態漏れの検出と封じ込めは [`flakiness-concurrency.md`](../flakiness-concurrency/SKILL.md) を参照)。
...
6. **順序非依存を確認する**:テスト順をシャッフルして走らせ([`flakiness-concurrency.md`](../flakiness-concurrency/SKILL.md))、緑のままなら setup/teardown が状態を正しく閉じている。
...
テストデータをどこまで本物に寄せるか、どこを既定値で隠すかの方針は [`strategy-operations.md`](../strategy-operations/SKILL.md) のテストデータ戦略に従う。
...
テスト本体の内部構成(AAA / Given-When-Then / テーブル駆動)は [`test-structure-anatomy.md`](../test-structure-anatomy/SKILL.md) を参照。
TDD の進め方(駆動のサイクルと戦略)は [`tdd-cycle.md`](../tdd-cycle/SKILL.md) を参照。
