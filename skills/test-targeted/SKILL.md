...
name: test-targeted
description: >
  テスト実行時の絞り込み運用ルール。プロジェクト全体テストではなく修正範囲に絞ったテスト実行を行う。
  `gradlew test`/`pytest`/`jest`/`go test`等のテストランナーを実行するとき、テスト実行コマンドを
  組み立てるときに必ず参照する。コミット差分や修正ファイルから対象テストを特定する手順を含む。
---
...
実行コマンドと生出力（終了コード・緑赤の集計行）を応答に提示して完了とする。貼れないなら未完。
