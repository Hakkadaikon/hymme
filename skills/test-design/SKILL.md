...
| テスト戦略(配分ピラミッド/トロフィー、関心事の階層化、リスクベース、シフトレフト/ライト、環境・データ戦略、CI ゲーティング) | [`reference/test-strategy.md`](reference/test-strategy.md) |
| 仕様から機械的に導く、入力空間の分割と履歴(同値分割、境界値、ドメイン分析、デシジョンテーブル、状態遷移、CRUD/ライフサイクル) | [`reference/blackbox-systematic.md`](reference/blackbox-systematic.md) |
| 条件や因子の組合せを縮約する(原因結果グラフ、ペアワイズ、直交表、T-way、クラシフィケーションツリー) | [`reference/blackbox-combinatorial.md`](reference/blackbox-combinatorial.md) |
| 経験や業務フローから導く(ユースケース、シナリオ、構文テスト、エラー推測、ファジング、探索的、アドホック) | [`reference/blackbox-experience.md`](reference/blackbox-experience.md) |
| 構造網羅やカバレッジ(C0/C1、条件、判定/条件、MC/DC、多重条件、パス、基底パス、ループ、データフロー) | [`reference/whitebox-coverage.md`](reference/whitebox-coverage.md) |
...
| テストが flaky(時刻/乱数/並行/順序/外部/タイマー/浮動小数の揺らぎを検出し封じ込める) | [`reference/test-flakiness.md`](reference/test-flakiness.md) |
...
| 非機能・品質特性(性能、負荷、a11y、セキュリティ、互換、信頼性、カオス、contract、データ移行…) | [`reference/nonfunctional-quality.md`](reference/nonfunctional-quality.md) |
...
- **「緑」の報告だけで完了にしない。** 実行ログと型検査を自分で確認する(上の実行ゲート)。flaky は1回の緑では見えないので、揺らぎを含むテストは複数回回す。flaky の要因別の検出と封じ込めは [`reference/test-flakiness.md`](reference/test-flakiness.md) に体系化してある。
- **API 境界の互換性維持を抜かさない。** 提供側と利用側を別々にテストするだけでは破壊的変更を見逃す。consumer-driven contract([`reference/nonfunctional-quality.md`](reference/nonfunctional-quality.md) のコントラクトテスト、粒度の選択は [`reference/levels.md`](reference/levels.md))で境界の互換を固定する。
