...
基本系（C0、C1、条件網羅、判定/条件網羅）は [whitebox-controlflow-basic.md](whitebox-controlflow-basic.md) を参照。

カバレッジは到達の指標であって検証の指標ではない。その溝を埋める手法は [mutation-testing.md](mutation-testing.md) で扱う。
...
基本系（C0、C1、条件網羅、判定/条件網羅）は [whitebox-controlflow-basic.md](whitebox-controlflow-basic.md) を参照。
ループとデータフローは [whitebox-dataflow-loop.md](whitebox-dataflow-loop.md)、テスト自身の欠陥検出力は [mutation-testing.md](mutation-testing.md) で扱う。
