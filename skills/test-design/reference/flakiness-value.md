...
**協調の非決定性**(並行・競合、テスト間順序・状態漏れ、外部ネットワーク、タイマー・sleep)と、両者に共通する**総論**(隔離 quarantine、retry 緑詐称の戒め、検出の3軸)は [`flakiness-concurrency.md`](flakiness-concurrency.md) を参照。
...
- property-based / メタモルフィックな seed 反復は [`good-test-principles.md`](good-test-principles.md) / [`test-design-quality.md`](test-design-quality.md) の脆さの議論と接続する。失敗 seed の最小反例の固定は [`generative-property.md`](generative-property.md)。
- 協調の非決定性と総論は [`flakiness-concurrency.md`](flakiness-concurrency.md)。
...
3. **property-based で網羅探索する**:fast-check 等で多数ケースを生成し、失敗時に出る seed と最小反例を回帰として固定する([`generative-property.md`](generative-property.md))。`fc.assert(prop, { numRuns: 1000 })` のように実行数を上げる。
...
> 隔離(quarantine)・retry による緑詐称の戒め・検出の3軸(seed/順序/並列)といった**共通の総論**は [`flakiness-concurrency.md`](flakiness-concurrency.md#共通方針隔離検出の総論) にまとめてある。値の非決定性を封じ込めたら、そちらも併せて確認する。
