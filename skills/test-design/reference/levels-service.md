...
サービス内部の粒度(単体、結合)は [levels.md](levels.md)、システム全体を外から見るレベルは [levels-system.md](levels-system.md) / [levels-operational.md](levels-operational.md) を参照。
...
2. **外部依存を洗い出してスタブ境界を引く**:サービスが呼ぶプロセス外依存(決済 API・他サービス・外部キュー)を列挙し、そこだけをスタブ化対象にする。内部のドメインロジックやサービス内 DB は本物のまま結線する([`test-doubles.md`](test-doubles.md) の振り分け)。
...
サービス内部の粒度(単体、結合)は [levels.md](levels.md)、配分・階層化は [`strategy-allocation.md`](strategy-allocation.md) を参照。
非機能の観点としてのコントラクト(互換性維持の運用)は [`nonfunctional-attributes.md`](nonfunctional-attributes.md) にもある。
