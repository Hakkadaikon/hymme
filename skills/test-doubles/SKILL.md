---
name: test-doubles
description: >
  テストダブルの五分類(ダミー、スタブ、モック、スパイ、フェイク)の役割整理と、
  実ミドルウェアを使い捨てコンテナで検証する Testcontainers の使い分けを扱う。
  test-catalog の手法カタログの一部。スタブとモックの役割を混同している、
  何をモック化すべきか迷っている、フェイクや Testcontainers をいつ使うか判断したい
  ときに使う。通常は test-catalog スキルの索引経由で手法が選定された後にこのスキルを直接参照する。
disable-model-invocation: true
---

# テストダブル

本物の協力者の代わりに差し込む偽物(テストダブル)の種類と使い分けをまとめる。
出典は Gerard Meszaros の用語を Vladimir Khorikov が整理したもの。
TDD の手順は [`tdd-cycle.md`](../tdd-cycle/SKILL.md)(サイクルの運用)と [`tdd-green-strategy.md`](../tdd-green-strategy/SKILL.md)(Green の戦略)、良いテストの規範(特にモック濫用の回避)は [`good-test-principles.md`](../good-test-principles/SKILL.md) を参照。
ここはダブルの**役割整理**(各種別が何を担うか)を扱う。どの依存をモックにしてよいかの**配置原則**(管理下/管理外、プロセス内/外)は [`test-design-quality.md`](../test-design-quality/SKILL.md) のモック濫用の回避にある。

各項目は「概要」「目的/いつ使う」「TypeScript example(vitest 想定)」「落とし穴」の構成で示す。

## 目次

- [ダミー、スタブ、モック、スパイ、フェイク](#ダミースタブモックスパイフェイク)
- [Testcontainers](#testcontainers)

---

## ダミー、スタブ、モック、スパイ、フェイク

### 概要
テストダブルの五分類(Gerard Meszaros の用語を Khorikov が整理したもの)。
本物の協力者の代わりに差し込む偽物の総称がテストダブルである。

- **ダミー(Dummy)**：渡すだけで使われない値。`null` や空オブジェクトなど、引数を埋めるためだけのもの。
- **スタブ(Stub)**：テスト対象へ入力を供給する。あらかじめ用意した戻り値を返す。受信側であり、検証はしない。
- **モック(Mock)**：テスト対象から外部への呼び出し(出力)を検証する。送信側であり、呼び出されたことを確かめる。
- **スパイ(Spy)**：モックと同じ役割を手書きで実装したもの。呼び出しを記録して後で検証する。
- **フェイク(Fake)**：本物相当の軽量な実装。インメモリのリポジトリなど。

### 目的/いつ使う
スタブとモックは役割で截然と分ける。
**スタブは入力を供給する受信側で、決して検証しない**。
**モックは外部への出力を検証する送信側で、`toHaveBeenCalledWith` 等で呼び出しを確かめる**。
スタブに対して呼び出し検証を行うのは過剰検証(over-specification)であり、脆さの主因になる。
一つのダブルがスタブ(戻り値供給)とモック(呼び出し検証)を兼ねる設計は避ける。

### TypeScript example
```ts
import { describe, it, expect, vi } from "vitest";
import { PriceQuote } from "./price-quote";

it("stub supplies input, mock verifies output", () => {
  // スタブ: 入力を供給するだけ。戻り値を用意し、呼び出しは検証しない
  const rateApi = { rateOf: vi.fn().mockReturnValue(150) };

  // モック: 外部への通信を検証する送信側
  const auditLog = { record: vi.fn() };

  const quote = new PriceQuote(rateApi, auditLog);
  const result = quote.inUsd(300);

  // スタブが供給した値の結果は「状態/出力」で確かめる(スタブ自体は検証しない)
  expect(result).toBe(2);
  expect(auditLog.record).toHaveBeenCalledWith("quote", 2);
});
```

### 落とし穴
- スタブに `toHaveBeenCalled` を掛けて検証し、入力供給にすぎないものを過剰に固定する。
- フェイクの挙動が本物と乖離し、テストは緑でも本番で壊れる。フェイクは本物の契約を守る。

### 遂行手順(着手→完了)
ダブルを選ぶ作業の本体は「テスト対象の各協力者を、入力前提か検証対象かへ振り分ける」ことだ。次の順で進める。

1. **協力者を列挙する**：テスト対象のコンストラクタ引数・メソッド引数・import している外部モジュールを全部書き出す。1 つも飛ばさない(飛ばした協力者は本物が走り、テストが環境に依存する)。
2. **各協力者を依存種別で仕分ける**：[`test-design-quality.md`](../test-design-quality/SKILL.md) のモック濫用の回避に従い、「管理外プロセス外(メール/外部 API/バス)」「管理下プロセス外(自前 DB/キュー)」「プロセス内(ドメインオブジェクト)」の 3 つへ振る。
3. **種別からダブルを割り当てる**：管理外プロセス外 → **モック**(出力として検証)、管理下プロセス外 → **フェイク**か本物([Testcontainers](#testcontainers))で状態検証、プロセス内 → **本物**。テスト対象へ値を供給するだけの協力者は **スタブ**(戻り値だけ用意、検証しない)。引数を埋めるだけで使われないものは **ダミー**。
4. **検証点を1テストにつき1つに絞る**：モックの呼び出し検証(出力)か、フェイク/本物の最終状態(状態)か、戻り値(出力)か、そのテストが見る「観察可能な振る舞い」を1つに決める。スタブには検証を掛けない。
5. **テストを書く**：スタブは `mockReturnValue` 等で戻り値を用意、モックは `vi.fn()` を渡して末尾で `toHaveBeenCalledWith`。1 協力者が戻り値供給と呼び出し検証を兼ねたら、テスト対象の責務が広すぎる信号なので分割を疑う。

### 完了チェック(もれ確認)
- **全協力者に種別が割り当たったか**：手順1で列挙した協力者の数と、手順3で「モック/フェイク/本物/スタブ/ダミー」を振った数が一致する。差があれば振り分け漏れ=本物が紛れて走っている。
- **スタブに検証が掛かっていないか**：`grep -n "toHaveBeenCalled" <test>` の各ヒットが、モックに割り当てた協力者だけに掛かっているかを目視で逆引きする。スタブに掛かっていれば過剰検証。
- **モックは管理外プロセス外だけか**：`toHaveBeenCalled*` を掛けた対象が、手順2で「管理外プロセス外」に分類したものだけであることを確認する。管理下 DB やプロセス内オブジェクトへのモック検証は脆さの証拠。
- **兼任ダブルが無いか**:同一ダブルに `mockReturnValue`(スタブ役)と `toHaveBeenCalled`(モック役)が両方掛かっていないか。両方あれば責務過多。

### 網羅設計での効かせ方
ダブルの選択は「何を網羅対象にし、何を差し替えるか」の線引きそのものである。
スタブは入力供給にすぎず網羅対象ではない。スタブ自体の呼び出しは検証しない。
モックは管理外のプロセス外依存への出力を網羅対象とし、呼び出しを検証する。
ゆえに「外部に出る副作用の組合せ」を網羅したいときはモックの検証点が網羅対象の集合を定める。
スタブに呼び出し検証を掛けると、入力供給まで網羅対象に紛れ込み、過剰検証で脆い網羅になる。
フェイク(インメモリ等)は管理下の依存を本物相当で網羅し、最終状態で検証する。
一つのダブルにスタブとモックを兼ねさせると、網羅対象(出力)と前提(入力)が混線して何を網羅したか不明になる。
よって網羅設計の最初の判断は、各協力者を「入力前提=スタブ/フェイク」と「網羅対象の出力=モック」へ振り分けることだ。

---

## Testcontainers

### 概要
データベースやメッセージブローカーなどの実ミドルウェアを、テスト実行のたびに使い捨てコンテナとして起動するライブラリ。

### 目的/いつ使う
管理下にあるプロセス外依存(自前 DB など)を、モックではなく本物で検証したいときに使う。
モック化はリポジトリ層の実装詳細へ結合するため、Khorikov は管理下の依存を本物でテストすることを勧める。
インメモリ偽物では SQL 方言やトランザクションの差で本番と乖離するが、Testcontainers は本番と同じエンジンを使うのでその乖離が出ない。
単体テストではなく結合テストの道具である点に注意する。

### TypeScript example
```ts
import { describe, it, expect, beforeAll, afterAll } from "vitest";
import { PostgreSqlContainer, StartedPostgreSqlContainer } from "@testcontainers/postgresql";
import { UserRepo } from "./user-repo";

let container: StartedPostgreSqlContainer;

beforeAll(async () => {
  container = await new PostgreSqlContainer("postgres:16").start();
}, 60_000);

afterAll(async () => {
  await container.stop();
});

it("persists and reads back a user (real Postgres)", async () => {
  const repo = await UserRepo.connect(container.getConnectionUri());
  await repo.save({ email: "a@example.com" });
  // 状態ベースで検証する。モックではないので実装詳細に結合しない
  expect(await repo.findByEmail("a@example.com")).toBeDefined();
});
```

### 落とし穴
- 速度が単体テストより桁違いに遅い。全テストを Testcontainers へ寄せず、結合テストの少数に絞る。
- コンテナの起動時間を見込まずタイムアウトで落ちる。起動に十分な制限時間を与える。

### 遂行手順(着手→完了)
1. **対象を結合テストに限定する**：振り分け(上の手順2)で「管理下プロセス外」に分類した依存のうち、フェイクでは契約の乖離(SQL 方言・トランザクション分離・制約)が出るものだけを Testcontainers の対象にする。単体で済む論理はここへ持ち込まない。
2. **本番と同じエンジン・メジャーバージョンを選ぶ**：`new PostgreSqlContainer("postgres:16")` のようにタグを本番に合わせる。ずれると乖離検出の意味が薄れる。
3. **ライフサイクルをスイート単位にする**：`beforeAll` で起動・`afterAll` で停止し、十分なタイムアウト(例 `60_000`)を与える。テストごとの起動は遅すぎる。
4. **各テストでデータを独立させる**：スイート共有コンテナなら、テストごとにスキーマを切るかトランザクションをロールバックして、テスト間の状態漏れを断つ([`flakiness-concurrency.md`](../flakiness-concurrency/SKILL.md) のテスト間順序・状態漏れ)。
5. **状態ベースで検証する**：保存→読み戻しの最終状態を見る。発行 SQL の文字列など実装詳細はアサートしない。

### 完了チェック(もれ確認)
- **対象が結合の代表に絞られているか**：Testcontainers を使うテスト数が、単体で取れる論理網羅まで巻き込んでいないか。広い同値分割・境界値は単体側にあることを確認する。
- **エンジン/バージョンが本番一致か**:コンテナタグと本番の DB バージョンが一致しているか(乖離していれば「本物で検証した」が嘘になる)。
- **テスト間が独立か**:順序をシャッフルしても緑か(`vitest --sequence.shuffle` 等)。落ちればデータ独立が崩れている。
- **起動失敗が握り潰されていないか**:CI でコンテナ起動エラーが skip 扱いになっていないか。skip された結合テストは守られていない。

### 網羅設計での効かせ方
Testcontainers は、管理下のプロセス外依存(自前 DB など)を本物のエンジンで網羅するための道具である。
インメモリの偽物で代用すると、SQL 方言やトランザクション分離レベルの差で網羅に穴が残り、本番と乖離する。
ゆえにリポジトリ層の「クエリ、制約、トランザクション境界」を網羅したいなら、本物エンジン上で網羅対象を踏む。
網羅対象は永続化の最終状態であり、状態ベースで検証する。実装詳細(発行 SQL の文字列)を網羅対象にしない。
ただし起動が遅いため、すべての網羅をここへ寄せない。
入力の同値分割や境界値といった広い網羅は単体テストで取り、ここは結合の代表ケースに絞る。
配分の原則は、単体テストで論理の網羅を厚く取り、Testcontainers では「本物の依存と本当に繋がるか」の少数の網羅に限ることだ。
