...
2. **各種別に差し替え方を割り当てる**：管理外プロセス外 → **モック**、管理下プロセス外 → **実物**(または [Testcontainers](test-doubles.md#testcontainers) の本物)、プロセス内 → **本物**。フェイクで足りる管理下依存はインメモリ実装で実物相当にする([`test-doubles.md`](test-doubles.md) の役割整理を参照)。
