...
- デプロイ前後の足切りや退行防止に使う運用確認の層(スモーク、サニティ、回帰)は [levels-operational.md](levels-operational.md) を読む。
...
レベルを選んだあとの配分(ピラミッド/トロフィー)と階層化(関心事による分割)は [`strategy-allocation.md`](strategy-allocation.md) を読む([`levels.md`](levels.md) 末尾と同じ)。
運用確認の層(スモーク/サニティ/回帰)は [levels-operational.md](levels-operational.md) を読む。
